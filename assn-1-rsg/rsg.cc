...
static void make_text(map<string, Definition>& grammar, Definition& defenition)
{
  Production expansion = defenition.getRandomProduction();
  for (vector<string>::iterator curr = expansion.begin(); curr != expansion.end(); ++curr) 
  {
    if (grammar.find(*curr) != grammar.end())
    {
      make_text(grammar, grammar[*curr]);
    }
    else{ printf("%s ", curr->c_str()); }
  }
}
...
int main(int argc, char *argv[])
{
  ...
  cout << "The grammar file called \"" << argv[1] << "\" contains " << grammar.size() << " definitions." << endl;
  
  make_text(grammar, grammar["<start>"]);
  printf(" \n");

...
typedef struct {
  hashset stopWords; 
  hashset indices;                 // is a rssIndexEntry
  vector previouslySeenArticles;  // is a rssNewsArticle
  Semaphore modifyPreviouslySeenArticles;
  Semaphore modifyIndices;
  Semaphore URLConnections;
  Semaphore modifyServerTracker;
  hashset serverTracker;
} rssDatabase;
...
typedef struct {
  ...
  vector relevantArticles;    // is a rssRelevantArticleEntry
} rssIndexEntry;
...
static void LoadStopWords(hashset *stopWords);
...
static void ScanArticle(streamtokenizer *st, int articleID, rssDatabase *db, hashset *stopWords);
...
static int serverTrackerHash(const void *elem, int numBuckets);
static int serverTrackerCompare(const void *elem1, const void *elem2);
static void serverTrackerFree(void *elem);
...
static const char *const kWelcomeTextFile = "assn-6-rss-news-search-data/welcome.txt";
static const char *const kDefaultStopWordsFile = "assn-6-rss-news-search-data/stop-words.txt";
static const char *const kDefaultFeedsFile = "assn-6-rss-news-search-data/rss-feeds-techy.txt";
...
int main(int argc, char **argv)
{
  ...
  LoadStopWords(&db.stopWords);

  InitThreadPackage(false);

  db.modifyPreviouslySeenArticles = SemaphoreNew("modArticles", 1);
  db.modifyIndices = SemaphoreNew("modIndices", 1);
  db.modifyServerTracker = SemaphoreNew("modServertracker", 1);
  db.URLConnections = SemaphoreNew("urlconnect", 24);
  HashSetNew(&db.serverTracker, sizeof(singleServerLimiter), 200, serverTrackerHash, serverTrackerCompare, serverTrackerFree);

  BuildIndices(&db, kDefaultFeedsFile);

  SemaphoreFree(db.modifyPreviouslySeenArticles);
  SemaphoreFree(db.modifyIndices);
  SemaphoreFree(db.modifyServerTracker);
  SemaphoreFree(db.URLConnections);
  HashSetDispose(&db.serverTracker);
...
static void LoadStopWords(hashset *stopWords)
{
  ...
  fp = fopen("assn-6-rss-news-search-data/stop-words.txt", "r");
  if(fp == NULL) 
 ...
 *                 rssFeedState, which contains an , which is the working
...
static void ProcessEndTag(void *userData, const char *name)
{
  ...
  if (strcasecmp(name, "item") == 0) 
  {
    ...
    ThreadNew("Threat", ParseArticle, 3, state->db, title, url);
  }
}
...
static void ParseArticle(rssDatabase *db, const char *articleTitle, const char *articleURL)
{
  ...
  streamtokenizer st;
  int articleID;
  singleServerLimiter serverCounter;
  char* newURL;
  int ID;
  char sID[20]; 
  ...
  SemaphoreWait(db->modifyPreviouslySeenArticles);
  int previouslySeen = VectorSearch(&db->previouslySeenArticles, &newsArticle, NewsArticleCompare, 0, false);
  SemaphoreSignal(db->modifyPreviouslySeenArticles);
  if (previouslySeen >= 0) {
  ...
  // search the ServerTracker for the Semaphore for this server:
  SemaphoreWait(db->modifyServerTracker);
  serverCounter.serverName = strdup(u.serverName);
  singleServerLimiter* existingServerEntry = HashSetLookup(&db->serverTracker, &serverCounter);
  if(existingServerEntry == NULL) {
    ID = HashSetCount(&db->serverTracker);

    snprintf(sID, sizeof(sID), "%d", ID);
    serverCounter.counter = SemaphoreNew(sID, 8);
    HashSetEnter(&db->serverTracker, &serverCounter);
    existingServerEntry = HashSetLookup(&db->serverTracker, &serverCounter);
    assert(existingServerEntry != NULL);
  }
  else{free(serverCounter.serverName);}
  SemaphoreSignal(db->modifyServerTracker);

  // limit the number of server connections to one server
  SemaphoreWait(existingServerEntry->counter);
  // limit the total number of server connections
  SemaphoreWait(db->URLConnections);
  URLConnectionNew(&urlconn, &u);
  ...
  switch (urlconn.responseCode) {
    ...
    SemaphoreSignal(db->modifyPreviouslySeenArticles);
		...
		ScanArticle(&st, articleID, db, &db->stopWords);
		STDispose(&st);
		break;
      case 301: 
      case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
           ...
           URLConnectionDispose(&urlconn);
           SemaphoreSignal(db->URLConnections);
           SemaphoreSignal(existingServerEntry->counter);
           ParseArticle(db, articleTitle, newURL);
           free(newURL);
		break;
...
{ 
  ...
  while (STNextToken(st, word, sizeof(word))) {
    ...
    {
      SemaphoreSignal(db->modifyIndices);
	    AddWordToIndices(&db->indices, word, articleID);
      SemaphoreSignal(db->modifyIndices);
    }
    }
  }
...
static int serverTrackerHash(const void *elem, int numBuckets)
{
  const singleServerLimiter *entry = elem;
  return StringHash(&entry->serverName, numBuckets);
}

static int serverTrackerCompare(const void *elem1, const void *elem2)
{
  const singleServerLimiter *entry1 = elem1;
  const singleServerLimiter *entry2 = elem2;
  return StringCompare(&entry1->serverName, &entry2->serverName);
}

static void serverTrackerFree(void *elem)
{
  singleServerLimiter *entry = elem;
  StringFree(&entry->serverName);
  SemaphoreFree(entry->counter);   
}
